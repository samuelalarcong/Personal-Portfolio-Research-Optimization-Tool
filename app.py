...
period = "5y"  # global — used by all analytics tabs
...
# Flag if portfolio contains crypto
CRYPTO_TICKERS = {"BTC-USD", "ETH-USD", "SOL-USD", "DOGE-USD", "XRP-USD", "ADA-USD", "BNB-USD"}
has_crypto  = any(t in CRYPTO_TICKERS for t in tickers)
...
with tab3:
    ...
    st.markdown(f"""
    > **Volatility** — how much the portfolio swings per year. Normal stocks: 15-25%. Bitcoin: 60-100%+.
    > **Sharpe** — return per unit of risk. Above 1.0 is good, above 2.0 is great.
    > **Max Drawdown** — worst peak-to-trough loss in this period.
    > **Beta** — sensitivity to {benchmark}. Above 1.0 = more volatile than the market.
    """)
...
# ══════════════════════════════════════════════════════
# TAB 7 — REBALANCING with EF + custom target
# ══════════════════════════════════════════════════════
...
with tab7:
    ...
    if "error" in opt_reb:
        ...
        # ── Efficient Frontier with interactive target ────────────────────
        st.markdown("#### Efficient frontier — pick your target")
        st.caption("Move the sliders to choose your desired risk/return point. The rebalancing orders below will update automatically.")

        ef = opt_reb.get("efficient_frontier", [])
        if ef:
            ef_df = pd.DataFrame(ef)
            vol_min  = float(ef_df["volatility"].min())
            vol_max  = float(ef_df["volatility"].max())
            ret_min  = float(ef_df["return"].min())
            ret_max  = float(ef_df["return"].max())

            sl1, sl2 = st.columns(2)
            with sl1:
                target_vol = st.slider(
                    "Target volatility %",
                    min_value=round(vol_min, 1),
                    max_value=round(vol_max, 1),
                    value=round(opt_reb["current_stats"]["volatility"], 1),
                    step=0.5,
            ...
            with sl2:
                target_ret = st.slider(
                    ...
                    value=round(opt_reb["current_stats"]["return"], 1),
                    step=0.5,
                    key="target_ret_slider",
                )

            # Find closest EF point to selected target
            ef_df["dist"] = ((ef_df["volatility"] - target_vol)**2 + (ef_df["return"] - target_ret)**2)**0.5
            closest_idx   = ef_df["dist"].idxmin()
            closest_vol   = ef_df.loc[closest_idx, "volatility"]
            closest_ret   = ef_df.loc[closest_idx, "return"]

            # Draw EF chart with target dot
            rebal_type = st.radio("Or use preset", ["Custom target ●", "Max Sharpe ◆", "Min Volatility ●"], horizontal=True, key="reb_preset")

            if rebal_type == "Max Sharpe ◆":
                selected_weights = opt_reb["max_sharpe_weights"]
                selected_label   = "Max Sharpe"
            elif rebal_type == "Min Volatility ●":
                selected_weights = opt_reb["min_vol_weights"]
                selected_label   = "Min Volatility"
            else:
                # Custom target: interpolate weights from closest EF point
                # Use the point on EF closest to the slider target
                # Weights: blend between min_vol and max_sharpe based on position
                ef_range = vol_max - vol_min if vol_max > vol_min else 1
                alpha    = (closest_vol - vol_min) / ef_range
                selected_weights = [
                    round(ms * alpha + mv * (1 - alpha), 2)
                    for ms, mv in zip(opt_reb["max_sharpe_weights"], opt_reb["min_vol_weights"])
                ]
                # Renormalize
                total_w = sum(selected_weights)
                selected_weights = [round(w / total_w * 100, 2) for w in selected_weights]
                selected_label   = "Custom target"
            ...
            fig_ef2.add_trace(go.Scatter(
                x=[opt_reb["current_stats"]["volatility"]],
                y=[opt_reb["current_stats"]["return"]],
                mode="markers", name="Current",
                marker=dict(color="#f59e0b", size=14, symbol="star"),
            ))
            fig_ef2.add_trace(go.Scatter(
                x=[opt_reb["max_sharpe_stats"]["volatility"]],
                y=[opt_reb["max_sharpe_stats"]["return"]],
                mode="markers", name="Max Sharpe",
                marker=dict(color="#34d399", size=12, symbol="diamond"),
            ))
            fig_ef2.add_trace(go.Scatter(
                x=[opt_reb["min_vol_stats"]["volatility"]],
                y=[opt_reb["min_vol_stats"]["return"]],
                mode="markers", name="Min Volatility",
                marker=dict(color="#f87171", size=12, symbol="circle"),
            ))
            # Target dot (purple)
            fig_ef2.add_trace(go.Scatter(
                x=[closest_vol], y=[closest_ret],
                mode="markers", name=f"Your target",
                marker=dict(color="#a855f7", size=16, symbol="circle",
            ...
            st.plotly_chart(fig_ef2, use_container_width=True, key="chart_ef_reb")

            # Store selected weights in session state for New Capital tab
            import streamlit as st_inner
            st.session_state["selected_weights_reb"] = selected_weights
            st.session_state["selected_tickers_reb"] = opt_reb["tickers"]

        # ── Broker-ready orders based on selected target ──────────────────
        st.divider()
        st.markdown(f"#### Orders — {selected_label if ef else 'Max Sharpe'}")
        st.caption("⚠️ Prices from Yahoo Finance last close. Verify in your broker before executing.")

        optimal_w  = selected_weights if ef else opt_reb["max_sharpe_weights"]
        ...
        if not sells and not buys:
            st.success("✅ Portfolio is already at the target. No rebalancing needed.")
        else:
            if sells:
...
""", unsafe_allow_html=True)
            ...
            sm4.metric("Net cash", f"${abs(net):,.2f}", "released" if net > 0 else "needed")
            ...
            ord_df.columns = ["Action","Ticker","Shares","Price $","Total $","Before","After","Weight Before","Weight After","Weight Δ"]
            ...
            def clr_a(val):
                ...
                return ""
            def clr_d(val):
                if isinstance(val, float): return f"color: {'#34d399' if val > 0 else '#f87171'}"
                return ""

            st.dataframe(
                ord_df.style
                    .format({"Price $":"${:,.2f}","Total $":"${:,.2f}","Weight Before":"{:.2f}%","Weight After":"{:.2f}%","Weight Δ":"{:+.2f}%"})
                    .applymap(clr_a, subset=["Action"])
                    .applymap(clr_d, subset=["Weight Δ"]),
            ...
            fig_reb2 = go.Figure()
            fig_reb2.add_trace(go.Bar(name="Before", x=tkrs_reb, y=opt_reb["current_weights"], marker_color="#94a3b8"))
            fig_reb2.add_trace(go.Bar(name="Target", x=tkrs_reb, y=optimal_w, marker_color="#6366f1"))
            fig_reb2.update_layout(
                barmode="group", yaxis_title="Weight %",
                paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)",
                font=dict(color="#e2e8f0"), margin=dict(t=20, b=20),
                yaxis=dict(gridcolor="#2a2d3e"), legend=dict(orientation="h", y=1.1),
            )
            st.plotly_chart(fig_reb2, use_container_width=True, key="chart_reb_bar")
...
with tab8:
    ...
    with i2:
        optimize_for = st.radio("Strategy", ["Max Sharpe", "Min Volatility", "Custom (from Rebalancing tab)"], horizontal=True)
    with i3:
        st.markdown("&nbsp;")
    ...
    if run_btn:
        ...
        if "error" in opt_cap:
            ...
            # Use target weights from Rebalancing tab if set, otherwise use preset
            if optimize_for == "Custom (from Rebalancing tab)" and "selected_weights_reb" in st.session_state:
                optimal_w_cap = st.session_state["selected_weights_reb"]
                # Align with opt_cap tickers
                reb_tickers = st.session_state.get("selected_tickers_reb", opt_cap["tickers"])
                if reb_tickers == opt_cap["tickers"]:
                    st.info("Using your custom target from the Rebalancing tab EF chart.")
                else:
                    optimal_w_cap = opt_cap["max_sharpe_weights"]
            elif optimize_for == "Max Sharpe":
                optimal_w_cap = opt_cap["max_sharpe_weights"]
            else:
                optimal_w_cap = opt_cap["min_vol_weights"]
