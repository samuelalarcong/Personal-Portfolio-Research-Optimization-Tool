...
# No-crypto versions — used for risk, optimization, rebalancing, new capital
positions_nc = positions[~positions["ticker"].isin(CRYPTO_TICKERS)].copy()
tickers_nc   = tuple(positions_nc["ticker"].tolist())
# Renormalize weights to 100% after removing crypto
total_nc     = positions_nc["current_value"].sum()
weights_nc   = tuple(round((v / total_nc) * 100, 2) for v in positions_nc["current_value"])
...
with tab3:
    ...
    if has_crypto:
        crypto_in_portfolio = [t for t in tickers if t in CRYPTO_TICKERS]
        st.warning(
            f"⚠️ This portfolio contains **{', '.join(crypto_in_portfolio)}**. "
            f"Crypto assets are highly volatile (60-100%+ annualised vol) and significantly inflate "
            f"portfolio-level risk metrics. The numbers below reflect the **real** combined risk "
            f"including crypto."
        )

    risk = get_risk(tickers, weights, benchmark, period)

    # ── Full portfolio risk (including crypto) ───────────────────────────
    st.markdown("##### Full portfolio (including crypto)")
    ...
    # ── Without crypto comparison ─────────────────────────────────────────
    if has_crypto:
        st.markdown("##### Without crypto (stocks only)")
        st.caption("Same portfolio but excluding BTC-USD and other crypto — shows the equity sleeve in isolation.")
        risk_nc = get_risk(tickers_nc, weights_nc, benchmark, period)
        nc1, nc2, nc3, nc4 = st.columns(4)

        vol_delta   = round(risk_nc["volatility"]   - risk["volatility"],   2)
        sharpe_delta = round(risk_nc["sharpe"]       - risk["sharpe"],       2)
        dd_delta    = round(risk_nc["max_drawdown"]  - risk["max_drawdown"], 2)

        nc1.metric("Volatility (ann.)",    f"{risk_nc['volatility']}%",   f"{vol_delta:+.2f}%")
        nc2.metric("Sharpe Ratio",         f"{risk_nc['sharpe']}",         f"{sharpe_delta:+.2f}")
        nc3.metric("Max Drawdown",         f"{risk_nc['max_drawdown']}%",  f"{dd_delta:+.2f}%")
        nc4.metric(f"Beta vs {benchmark}", f"{risk_nc['beta']}" if risk_nc['beta'] else "—")

        st.info(
            f"Removing crypto reduces volatility from **{risk['volatility']}%** → **{risk_nc['volatility']}%** "
            f"and improves Sharpe from **{risk['sharpe']}** → **{risk_nc['sharpe']}**. "
            f"This shows how much BTC alone is driving the portfolio risk."
        )
...
with tab6:
    ...
    if has_crypto:
        st.info("ℹ️ Crypto (BTC-USD etc.) is excluded from optimization — it distorts mean-variance math. Holdings and P&L still include it.")
    with st.spinner("Running Markowitz optimization..."):
        opt = run_optimization(list(tickers_nc), list(weights_nc), period)
    ...
    if "error" in opt:
        ...
        with st.spinner("Analysing each position..."):
            ra = risk_adjusted_analysis(list(tickers_nc), period)
...
with tab7:
    ...
    if has_crypto:
        st.info("ℹ️ Crypto excluded from rebalancing optimization — stocks only.")
    with st.spinner("Running optimization for rebalancing..."):
        opt_reb = run_optimization(list(tickers_nc), list(weights_nc), period)
...
with tab8:
    ...
    if run_btn:
        with st.spinner("Running optimization and calculating share counts..."):
            opt_cap = run_optimization(list(tickers_nc), list(weights_nc), period)
